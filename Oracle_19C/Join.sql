-- Joins compativeis com SQL ANSI 1999 - Compativeis com qualquer Banco de Dados
1 - NATURAL JOIN
2 - USING
3 - ON
...
-- Utilizando Alias de Tabela -- Apelidando um Tabela
SELECT
	   e.employee_id,
	   e.last_name,
	   e.department_id,
...
SELECT
	   ...
	   location_id, city
FROM    departments
NATURAL JOIN locations; -- SQL Identifica as colunas iguais entre as duas Tabelas e realiza o relacionamento
...
SELECT
		...
		d.department_name
FROM employees e
JOIN departments d USING (department_id); -- É explicito a coluna que será usada para o relacionamento
...
SELECT
	   ...
	   e.department_id,
	   d.location_id
FROM employees e JOIN departments d ON (e.department_id = d.department_id); -- Explicito entre as duas tabelas as colunas que ligam/amarram a relação
...
SELECT
	   ...
	   d.department_name
FROM employees e
JOIN departments d ON (e.department_id = d.department_id) AND (e.salary BETWEEN 10000 AND 15000);
